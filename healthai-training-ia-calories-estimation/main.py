...
# Imports locaux
from config import (
    CSV_FILE, SCHEMA, FEATURES_COLS, TARGET_COL,
    CATEGORICAL_COLS, TRAIN_TEST_SPLIT_RATIO, RANDOM_STATE,
    NORMALIZE_NUMERIC, SCALING_METHOD,
    RF_PARAMS, GB_PARAMS, METRICS_THRESHOLDS,
    DATA_MODELS_DIR, MODEL_NAME, VERSION_MAJOR
)
...
def main():
    ...
    try:
        ...
        logger.info(f"✅ Preprocessing terminé ({X_train.shape[0]} train / {X_test.shape[0]} test)")
        ...
        logger.info(f"✅ Évaluation terminée - Meilleur : {comparison['summary']['best_overall']}")
        ...
        rf_model_path = save_model(rf_model, "RandomForest", version_dir, "random_forest")
        gb_model_path = save_model(gb_model, "GradientBoosting", version_dir, "gradient_boosting")
        ...
        data_paths = save_training_data(X_train, X_test, y_train, y_test, df_raw, version_dir)
        ...
        logger.info(f"Meilleur modèle : {comparison['summary']['best_overall'].upper()}")
        logger.info(f"RF: {comparison['summary']['random_forest']} points | GB: {comparison['summary']['gradient_boosting']} points")
        logger.info("="*80)
