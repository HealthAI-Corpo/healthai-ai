...
async def generate_session(
    ...
    background_tasks.add_task(
        ...
        lambda db: ai_session_service.generate_session(
            db=db, user_id=x_user_id, contraintes=contraintes, sauvegarder=sauvegarder
        ),
    )
...
async def evaluate_sessions(
    ...
    background_tasks.add_task(
        ...
        lambda db: ai_session_service.evaluate_sessions_by_ids(
            db=db, user_id=x_user_id, ids_seances=ids
        ),
    )
    return JobCreatedResponse(job_id=job_id)
...
async def evaluate_my_recent_sessions(
    ...
    background_tasks.add_task(
        job_service.run_in_background,
        job_id,
        lambda db: ai_session_service.evaluate_recent_sessions(db=db, user_id=x_user_id),
    )
    return JobCreatedResponse(job_id=job_id)
...
async def get_job_status(job_id: str, _: None = Depends(require_mongo)) -> JobStatusResponse:
